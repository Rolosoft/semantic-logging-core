...
using Microsoft.Practices.EnterpriseLibrary.SemanticLogging.Tests.Utility;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using Rolosoft.Practices.EnterpriseLibrary.SemanticLogging.Configuration;
using Rolosoft.Practices.EnterpriseLibrary.SemanticLogging.Etw;
using Rolosoft.Practices.EnterpriseLibrary.SemanticLogging.Etw.Configuration;
...
// Do not run on build server
    ...
    public abstract class given_traceEventService : ContextBase
    {
        ...
        protected override void Given()
        {
            ...
            inMemoryListener = new InMemoryEventListener(formatter);
            var sink = new Lazy<IObserver<EventEntry>>(() => inMemoryListener);
            sourceSettings = sourceSettings ?? new EventSourceSettings(EventSource.GetName(typeof(MyCompanyEventSource)));
            ...
            configuration = new TraceEventServiceConfiguration(sinkSettings, serviceSettings);

            try
            {
                sut = new TraceEventService(configuration);
            }
        ...
        protected bool IsCreatedSessionAlive()
        {
            return TraceEventSession.GetActiveSessionNames().Any(s =>
                s.StartsWith(configuration.Settings.SessionNamePrefix, StringComparison.OrdinalIgnoreCase));
        }
...
// Do not run on build server
    [Ignore]
        ...
        public class when_starting_session : given_traceEventService
        {
            ...
            {
                Assert.AreEqual(ServiceStatus.Started, sut.Status);
...
// Do not run on build server
    [Ignore]
        ...
        public class when_stopping_session : given_traceEventService
        {
            ...
            {
                Assert.AreEqual(ServiceStatus.Stopped, sut.Status);
...
// Do not run on build server
    [Ignore]
        ...
        public class when_disposing_session : given_traceEventService
        {
            ...
            {
                Assert.AreEqual(ServiceStatus.Disposed, sut.Status);
                Assert.IsFalse(IsCreatedSessionAlive());
                sut.Start();
            }
        }
...
// Do not run on build server
    [Ignore]
        ...
        public class when_starting_second_service_instance_with_same_config_other_sessionPrefix : given_traceEventService
        {
            ...
            protected override void When()
            {
                configuration.Settings.SessionNamePrefix = "SecondInstance";
                collector2 = new TraceEventService(configuration);
            }
        ...
        public class when_logging_an_event : given_traceEventService
        {
            ...
            public void then_event_is_collected_and_processed()
            {
                ...
                inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout);
        ...
        public class when_logging_an_event_with_activity_id_and_related_activity_id : given_traceEventService
        {
            ...
            protected override void When()
            {
                ...
                try
                {
                    ...
                    MyCompanyEventSource.Log.WithRelatedActivityId(relatedActivityId);
                }
                finally
                {
                    //EventSource.SetCurrentThreadActivityId(previousActivityId);
            ...
            public void then_event_is_collected_and_processed()
            {
                ...
                Assert.AreEqual(activityId, entry.ActivityId);
                Assert.AreEqual(relatedActivityId, entry.RelatedActivityId);
            }
        }
...
// Do not run on build server
    [Ignore]
        ...
        public class when_logging_an_event_by_keywords : given_traceEventService
        {
            ...
            {
                // Wait for event to be processed
                Assert.IsTrue(inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout));
    ...
    [Ignore]
        ...
        public class when_logging_from_two_different_eventSources : given_traceEventService
        {
            protected override void Given()
            {
                ...
                eventSources.Add(localSourceSettings);
                sinkSettings = new List<SinkSettings> { new SinkSettings("test", sink, eventSources) };
                configuration = new TraceEventServiceConfiguration(sinkSettings);
                sut = new TraceEventService(configuration);
                sut.Start();
    ...
    [Ignore]
        ...
        public class when_logging_to_provider_created_in_two_different_sessions : given_traceEventService
        {
            ...
            protected override void Given()
            {
                ...
                sut2 = new TraceEventService(localConfiguration);
            }
            ...
            public void then_event_is_collected_and_processed_in_first_session()
            {
                // Wait for event to be processed
                inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout);
                inMemoryListener2.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout);
...
// Do not run on build server
    [Ignore]
        ...
        public class when_tryToUpdateConfiguration_with_new_level : given_traceEventService
        {
            ...
            protected override void When()
            {
                var current = configuration.SinkSettings[0];
                ...
                configuration.SinkSettings.Remove(current);
                configuration.SinkSettings.Add(newSink);
            }
    ...
    [Ignore]
        ...
        public class when_tryToUpdateConfiguration_with_new_listener : given_traceEventService
        {
            ...
            protected override void When()
            {
                ...
                var localEventSources = new List<EventSourceSettings> { localSourceSettings };
                configuration.SinkSettings.Add(new SinkSettings("test2", sink, localEventSources));
            }
            ...
            public void then_new_configuration_is_updated()
            {
                ...
                // Wait for event to be processed
                Assert.IsTrue(inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout), "wait timed out");
...
// Do not run on build server
    [Ignore]
        ...
        public class when_updating_manifest_and_logging_a_new_event : given_traceEventService
        {
            ...
            protected override void Given()
            {
                domain1 = new DisposableDomain();
                domain2 = new DisposableDomain();

                var initialManifest = EventSource.GenerateManifest(typeof(MyNewCompanyEventSource), null);
                sourceSettings = new EventSourceSettings(EventSource.GetName(typeof(MyNewCompanyEventSource)));
                base.Given();
            ...
            {
                // Wait for event to be processed
                bool signaled = inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout);
...
// Do not run on build server
    [Ignore]
        ...
        public class when_logging_an_event_with_multiple_payload_types : given_traceEventService
        {
            protected override void Given()
            {
                sourceSettings = new EventSourceSettings(EventSource.GetName(typeof(MultipleTypesEventSource)));
                base.Given();
                sut.Start();
            }
...
// Do not run on build server
    [Ignore]
        ...
        public class when_logging_with_multiple_enum_types : given_traceEventService
        {
            ...
            protected override void Given()
            {
                sourceSettings = new EventSourceSettings(EventSource.GetName(typeof(DifferentEnumsEventSource)));
                base.Given();
...
// Do not run on build server
    [Ignore]
        ...
        public class when_logging_with_a_provider_with_large_manifest : given_traceEventService
        {
            protected override void Given()
            {
                sourceSettings = new EventSourceSettings(EventSource.GetName(typeof(LargeManifestEventSource)));
                base.Given();
                sut.Start();
            }
        ...
        public class when_logging_an_event_from_a_process_process_included_in_filters : given_traceEventService
        {
            protected override void Given()
            {
                sourceSettings =
                    ...
                    processNameFilters: new[] { Path.GetFileName(Process.GetCurrentProcess().MainModule.FileName) });
                base.Given();
                sut.Start();
            }
...
// Do not run on build server
    [Ignore]
        ...
        public class when_logging_an_event_from_a_process_process_not_included_in_filters : given_traceEventService
        {
            ...
            {
                // Wait for event to be processed
                Assert.IsFalse(inMemoryListener.WaitOnAsyncEvents.WaitOne(AsyncProcessTimeout));
    ...
    [Ignore]
        ...
        public class when_logging_multiple_events_from_a_process_process_without_sampling : given_traceEventService
        {
            protected override void Given()
            {
                sourceSettings =
                    new EventSourceSettings(
                        EventSource.GetName(typeof(MyCompanyEventSource)));
                base.Given();
                inMemoryListener.WaitSignalCondition = () => inMemoryListener.EventWrittenCount == 15;
                sut.Start();
            }
            ...
            public void then_all_events_are_collected_and_processed()
            {
                ...
                Assert.AreEqual(15, inMemoryListener.EventWrittenCount);
                Assert.AreEqual(0, formatter.WriteEventCalls[0].Payload[0]);
                Assert.AreEqual(1, formatter.WriteEventCalls[1].Payload[0]);
                Assert.AreEqual(2, formatter.WriteEventCalls[2].Payload[0]);
                Assert.AreEqual(3, formatter.WriteEventCalls[3].Payload[0]);
                Assert.AreEqual(4, formatter.WriteEventCalls[4].Payload[0]);
                Assert.AreEqual(5, formatter.WriteEventCalls[5].Payload[0]);
                Assert.AreEqual(6, formatter.WriteEventCalls[6].Payload[0]);
                Assert.AreEqual(7, formatter.WriteEventCalls[7].Payload[0]);
                Assert.AreEqual(8, formatter.WriteEventCalls[8].Payload[0]);
                Assert.AreEqual(9, formatter.WriteEventCalls[9].Payload[0]);
                Assert.AreEqual(10, formatter.WriteEventCalls[10].Payload[0]);
                Assert.AreEqual(11, formatter.WriteEventCalls[11].Payload[0]);
                Assert.AreEqual(12, formatter.WriteEventCalls[12].Payload[0]);
                Assert.AreEqual(13, formatter.WriteEventCalls[13].Payload[0]);
                Assert.AreEqual(14, formatter.WriteEventCalls[14].Payload[0]);
            }
        }
    ...
    [Ignore]
        ...
        public class when_logging_multiple_events_from_a_process_process_with_sampling : given_traceEventService
        {
            ...
            public void then_sampled_events_are_collected_and_processed()
            {
                ...
                Assert.AreEqual(3, inMemoryListener.EventWrittenCount);
                Assert.AreEqual(0, formatter.WriteEventCalls[0].Payload[0]);
                Assert.AreEqual(5, formatter.WriteEventCalls[1].Payload[0]);
                Assert.AreEqual(10, formatter.WriteEventCalls[2].Payload[0]);
            }
        }
#endif
    }
