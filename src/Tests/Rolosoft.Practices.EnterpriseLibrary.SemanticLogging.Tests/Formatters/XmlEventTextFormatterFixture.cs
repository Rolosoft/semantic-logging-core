    ...
    [Ignore]
        ...
        public class when_receiving_event_with_payload_and_message : given_xml_event_text_formatter
        {
            ...
            protected override void When()
            {
                ...
                this.threadId = Microsoft.Practices.EnterpriseLibrary.SemanticLogging.Tests.Utility.NativeMethods.GetCurrentThreadId();
    ...
    [Ignore]
        ...
        public class when_writing_to_null_writer : given_xml_event_text_formatter
        {
            ...
            protected override void When()
            {
                try
                {
                    formatter.WriteEvent(new EventEntry(Guid.NewGuid(), 0, string.Empty, new System.Collections.ObjectModel.ReadOnlyCollection<object>(new object[0]), DateTimeOffset.MaxValue, new Schema.EventSourceSchemaReader().GetSchema(logger).Values.First()), null);
                    Assert.Fail("should have thrown");
                }
                catch (ArgumentNullException e)
                {
